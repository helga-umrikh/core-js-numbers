...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const firstMagn = Math.sqrt(x1 * x1 + y1 * y1);
  const secMagn = Math.sqrt(x2 * x2 + y2 * y2);
  const angle = Math.acos(dotProduct / (firstMagn * secMagn));
  return angle;
}
...
function roundToPowerOfTen(num, pow) {
  ...
  const powRounded = Math.round(num / powResult);
  const result = powRounded * powResult;
  return result;
}
...
function isNumber(number) {
  if (!number.isNaN && typeof number === 'number') {
    return true;
  }
...
function getFloatOnString(str) {
  ...
  return parsedFloat.isNaN ? NaN : parsedFloat;
}
...
function getIntegerOnString(str, base) {
  ...
  return isInteger(parsedInt) ? NaN : parsedInt;
}
